    ...
    if (socket != null && socket!.connected) return;
      ...
      socket?.on(SocketEvents.updateStats, (data) {
        if (data is Map<String, dynamic>) {
          ...
          // Capture old rank BEFORE we update the notifier
          final oldRank = statsNotifier.value['currentRank'] ?? 'Unknown';

          // Update notifier with clean integers
          statsNotifier.value = {...statsNotifier.value, ...cleaned};

          // NEW: Simple rank-up detection using the server's currentRank
          final newRank = cleaned['currentRank'] ?? 'Unknown';
          if (newRank != oldRank && cleaned.containsKey('experience')) {
          ...
          // Keep your existing safety defaults and death check
          statsNotifier.value['bullets'] = statsNotifier.value['bullets'] ?? 0;
          statsNotifier.value['lastMidLevelOp'] = statsNotifier.value['lastMidLevelOp'] ?? 0;
          statsNotifier.value['overallPower'] = statsNotifier.value['overallPower'] ?? 0;
          statsNotifier.value['weapon'] = statsNotifier.value['weapon'] ?? null;
          ...
          final bool isDeadNow = (statsNotifier.value['dead'] == true) || 
                                (statsNotifier.value['health'] ?? 100) <= 0;
          if (isDeadNow) {
            deathNotifier.value = true;
          }
        }
      ...
      socket?.on('special-op-join-result', (data) {
        if (data is Map && data['success'] == true && data['party'] != null) {
          final partyData = Map<String, dynamic>.from(data['party']);

          // Update the main notifier (this triggers the ValueListenableBuilder in operations_screen.dart)
          specialOpPartyNotifier.value = partyData;

          // Keep statsNotifier in sync (backward compatibility + safety)
          final currentStats = Map<String, dynamic>.from(statsNotifier.value);
          currentStats['activeSpecialOperationParty'] = partyData;
          currentStats['activeSpecialOperation'] = partyData['operation'] ?? '';
          statsNotifier.value = currentStats;

          print('✅ Special op join confirmed — party UI updated live');
        }
      });
