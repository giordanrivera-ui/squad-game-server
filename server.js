...
setInterval(() => {
    ...
    if (expiry <= now) {
                ...
                clearCrimeFreezeForPlayer(db, perpetrator).catch(err => {
                    console.error(`[JUSTICE] Failed to auto-unfreeze ${perpetrator} after opportunity expired:`, err);
                });
            }
        }
...
socket.on('private-message', async (data) => {
  ...
  if (socket.data.displayName) {
    ...
    // === NEW: Clean up any active loot decision for this player ===
    ...
    // === FIX FOR BUG #1: Unfreeze the criminal if witness disconnects mid-decision ===
